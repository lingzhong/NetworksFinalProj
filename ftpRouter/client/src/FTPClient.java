    ...
    public void run() {
        try {
            while (!this.socket.isClosed()) {
                int ack = Integer.parseInt(this.reader.readLine());
...
class FTPClient {
    ...
    public static void main(String[] args) {
        ...
        try {
            // initialize TCP connection the other end
            Socket socket = null;
            ...
            BufferedReader reader = new BufferedReader(new InputStreamReader(
                    socket.getInputStream())); // for reading lines
            DataOutputStream writer = new DataOutputStream(
                    socket.getOutputStream()); // for writing lines.
            ...
            // Use StringTokenizer to store the values read from the server in
            // matrix
            ...
            System.out
                    .println("Enter the name of the file to transfer to server");
            ...
            for (int i = 0; i < packets.length; i++) {
                byte[] header = ByteBuffer.allocate(SIZE_OF_HEADER)
                        .putInt(i + 1).array();
                byte[] data = Arrays.copyOfRange(fBytes, i * SIZE_OF_DATA,
                        Math.min((i + 1) * SIZE_OF_DATA, fBytes.length));
                packets[i] = new byte[header.length + data.length];
                System.arraycopy(header, 0, packets[i], 0, header.length);
                System.arraycopy(data, 0, packets[i], header.length,
                        data.length);
            }

            // initialize sliding window protocol params
            ...
            // kicks off a seperate listener thread to receive and handle ACK
            ...
            while (packetNumber < noPackets) {
                ...
                for (int i = packetNumber; i < Math.min(packetNumber + cwnd,
                        noPackets); i++) {
                    writer.write(packets[i], 0, packets[i].length);
                    System.out.println("Pack number " + (i + 1) + " of size "
                            + packets[i].length + " sent");
                }
            ...
            System.out.println("Total time to send all packets: "
                    + (transferEndTime - transferStartTime) / 1000
                    + " seconds.");
            System.out.println("Total time in terms of RTT: " + RTTcount
                    + " RTT.");
            System.out.println(lastAck + " out of " + noPackets
                    + " packets have been sent successfully");

            // clean up resources
            socket.close();
            Thread.sleep(1); // wait for listener thread to complete the
                             // iteration
            reader.close();
            writer.close();
            t.join();
